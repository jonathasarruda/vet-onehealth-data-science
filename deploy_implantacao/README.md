...
## Enunciado (EN/PT)
**EN:** This folder contains deploy snippets for veterinary data science applied to One Health.  
**PT:** Esta pasta contém trechos de deploy para ciência de dados veterinária aplicada à Saúde Única.  
...
**EN:** A dashboard showing zoonotic risk distribution, an API responding at `/predict`, and a workflow that updates results weekly.  
**PT:** Um dashboard mostrando a distribuição de risco zoonótico, uma API respondendo em `/predict` e um workflow que atualiza os resultados semanalmente.  

## Tags
`streamlit` `fastapi` `ci/cd` `onehealth` `veterinary` `deploy` `bilingual`
